...
const { startUsdtBalanceMonitor, stopUsdtBalanceMonitor } = require('./services/usdtBalanceMonitor');
...
app.get('/api/webapp/config', (_req, res) => {
  res.status(200).json({ energyAddress: config.energyAddress, exchangeAddress: config.exchangeAddress });
});
...
app.get('/', (_req, res) => res.status(200).send('TRON 能量闪租机器人运行中'));
app.get('/health', (_req, res) => res.status(200).json({ status: 'ok' }));

async function start() {
  try {
    await initDatabase();
    await startUsdtBalanceMonitor();
    app.listen(config.port, '0.0.0.0', () => console.log(`健康检查服务已启动，监听端口 ${config.port}`));
    await bot.launch();
