...
bot.hears('⚡ TRX能量', (ctx) => {
  const text = ['⚡ TRX能量闪租', '', `✅ 1笔转账 = ${config.priceWithUsdt} TRX（对方地址有U）`, `✅ 1笔转账 = ${config.priceWithoutUsdt} TRX（对方地址无U）`, `💰 比直接转账节省约 ${config.feeSavePercent}% 手续费`, '', '付费能量地址：', config.energyAddress || 'ENERGY_ADDRESS 未配置', '', '👇 向以上地址转入对应数量的 TRX，系统将自动为你的目标地址闪租能量'].join('\n');
...
bot.hears('💱 U换TRX', (ctx) => {
  const text = ['💱 USDT 换 TRX', '', config.exchangeRateNote, '最低付款 1 USDT，最高 2000 USDT', '3秒完成兑换，请勿使用交易所提币', '', '兑换地址：', config.exchangeAddress, '', '👇 向以上地址转账，系统将自动完成兑换'].join('\n');
  return ctx.reply(text, keyboards.exchangeInlineKeyboard());
});
bot.hears('🎁 每天每人送5次能量', (ctx) => ctx.reply(['🎁 每天每人免费送5次能量', '', '👇 点击下方按钮进入小程序立即领取'].join('\n'), keyboards.freeEnergyInlineKeyboard()));
