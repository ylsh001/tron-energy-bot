...
bot.hears('⚡ TRX能量', (ctx) => {
  const text = [
    '⚡ TRX能量闪租',
    '',
    `✅ 1笔转账 = ${config.priceWithUsdt} TRX（对方地址有U）`,
    `✅ 1笔转账 = ${config.priceWithoutUsdt} TRX（对方地址无U）`,
    `💰 比直接转账节省约 ${config.feeSavePercent}% 手续费`,
    '',
    '👇 向下方地址转入对应数量的 TRX，系统将自动为你的目标地址闪租能量',
  ].join('\n');
...
bot.hears('💱 U换TRX', (ctx) => {
  const text = ['💱 U换TRX', '', config.exchangeRateNote, '', '👇 点击下方按钮联系客服办理兑换'].join(
    '\n'
  );
...
bot.hears('🎁 每天每人送5次能量', (ctx) => {
  const text = ['🎁 每天每人免费送5次能量', '', '👇 点击下方按钮进入小程序立即领取'].join('\n');
