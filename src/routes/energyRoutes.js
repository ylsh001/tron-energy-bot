...
router.post('/profile', async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({
      ...
      remainingToday,
      dailyLimit: config.freeEnergyDailyLimit,
    });
  } catch (err) {
...
router.post('/request', async (req, res) => {
  ...
  if (!JOB_ID_REGEX.test(runpodJobId)) {
  ...
  if (
    !job ||
    job.user_id !== identity.userId ||
    job.status !== 'COMPLETED' ||
    job.from_address !== fromAddress ||
    job.to_address !== toAddress
  ) {
  ...
  await updateRunpodJobUsdtBalance(runpodJobId, resultAddressUsdt);

  if (hadSuccessfulClaim && currentEnergy <= 0 && resultAddressUsdt <= 0) {
    await addUserToBlacklist({
      userId: identity.userId,
      firstName: identity.firstName,
      username: identity.username,
      reason: '历史领取地址能量已耗尽且本次返回地址无 USDT',
      source: 'automatic',
    });
    return res.status(403).json({ blocked: true });
  }
  ...
  const hasUsdt = resultAddressUsdt > 0;
