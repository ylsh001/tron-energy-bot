...
async function loadRecords() {
  ...
  try {
    const data = await readJson(await fetch(`/api/admin/records?limit=500&withUsdt=${requestedWithUsdt}`));
    ...
    statusText.textContent = requestedWithUsdt ? `当前有 USDT 的返回地址：${data.records.length} 条` : `已加载 ${data.records.length} 条`;
  } catch (err) {
...
async function loadMonitor() {
  try {
    ...
    renderMonitor(data.monitor || {});
    if (!data.monitor?.running && !manualRefreshPending) {
      refreshBtn.disabled = false;
      refreshBtn.textContent = '立即查询余额';
    }
  } catch (err) {
    monitorState.textContent = '监控状态读取失败';
...
async function refreshBalances() {
  ...
  refreshBtn.textContent = '正在查询...';
  try {
    const data = await readJson(await fetch('/api/admin/monitor/refresh', { method: 'POST' }));
    renderMonitor(data.monitor || {});
    await loadRecords();
  } catch (err) {
...
monitorPollingTimer = setInterval(loadMonitor, 5000);
window.addEventListener('beforeunload', () => clearInterval(monitorPollingTimer));
loadRecords();
loadBlacklist();
