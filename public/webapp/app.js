...
let progressTimer = null; let progressValue = 0; let pendingConfirmResolve = null;
function sleep(ms){return new Promise((resolve)=>setTimeout(resolve,ms));}
function notify(type){if(tg?.HapticFeedback)tg.HapticFeedback.notificationOccurred(type);}
async function writeToClipboard(text){if(!text)return false;try{if(navigator.clipboard&&window.isSecureContext){await navigator.clipboard.writeText(text);return true;}}catch(err){console.warn('navigator.clipboard 写入失败，尝试兼容方式',err);}try{const textarea=document.createElement('textarea');textarea.value=text;textarea.setAttribute('readonly','');textarea.style.position='fixed';textarea.style.left='0';textarea.style.top='0';textarea.style.width='1px';textarea.style.height='1px';textarea.style.opacity='0';document.body.appendChild(textarea);textarea.focus({preventScroll:true});textarea.select();textarea.setSelectionRange(0,text.length);const success=document.execCommand('copy');document.body.removeChild(textarea);return success;}catch(err){console.error('兼容方式写入剪贴板失败',err);return false;}}
function getRunpodAddress(result){const candidates=[result.address,result.generatedAddress,result.output?.address,result.output?.generatedAddress,result.data?.address];for(const value of candidates){if(typeof value==='string'&&TRON_ADDRESS_REGEX.test(value.trim()))return value.trim();}try{const matched=JSON.stringify(result).match(/T[1-9A-HJ-NP-Za-km-z]{33}/);return matched?matched[0]:'';}catch(_err){return '';}}
...
function startProgress(title,text){progressValue=0;progressBox.classList.remove('hidden');setProgress(3,title,text);clearInterval(progressTimer);progressTimer=setInterval(()=>{const next=progressValue<55?progressValue+Math.random()*7+2:progressValue<82?progressValue+Math.random()*3:progressValue+Math.random()*.8;setProgress(Math.min(next,92));},900);}
...
async function loadWebAppConfig(){try{const res=await fetch('/api/webapp/config');const data=await readJsonResponse(res);if(!res.ok||!data.exchangeAddress)throw new Error('兑换地址加载失败');exchangeAddressCopy.dataset.copy=data.exchangeAddress;exchangeAddressText.textContent=data.exchangeAddress;}catch(err){console.error('WebApp 配置加载失败',err);exchangeAddressText.textContent='兑换地址暂不可用';exchangeAddressCopy.disabled=true;}}
async function submitRunpodTask(toAddress){updateProgress('正在提交任务','正在生成领取任务，请保持页面打开...',8);const res=await fetch('/api/runpod/run',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({initData:tg.initData,toAddress})});const data=await readJsonResponse(res);if(!res.ok||!data.success)throw new Error(data.error||'领取任务提交失败，请稍后重试');updateProgress('已加入处理队列','系统正在为你安排领取任务，请保持页面打开...',18);return data;}
...
loadWebAppConfig();fromAddressInput.addEventListener('input',clearError);toAddressInput.addEventListener('input',clearError);
