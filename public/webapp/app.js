...
const energyAddressCopy = document.getElementById('energyAddressCopy');
const energyAddressText = document.getElementById('energyAddressText');
...
function showClaimNoticeModal() {
  claimNoticeModal.classList.remove('hidden');
  acceptClaimBtn.disabled = false;
  cancelClaimBtn.disabled = false;
  return new Promise((resolve) => {
    pendingNoticeResolve = resolve;
  });
}

function hideClaimNoticeModal() {
  claimNoticeModal.classList.add('hidden');
  acceptClaimBtn.disabled = false;
  cancelClaimBtn.disabled = false;
  pendingNoticeResolve = null;
}
...
async function showScreeningFailure() {
  ...
  await sleep(5000);
  finishProgress();
  showToast('能量领取失败，联系客服或者使用付费能量百分百成功', 5000);
  notify('error');
}
...
async function loadWebAppConfig() {
  try {
    ...
    const energyAddress = String(data.energyAddress || '').trim();
    if (energyAddress) {
      energyAddressCopy.dataset.copy = energyAddress;
      energyAddressText.textContent = energyAddress;
      energyAddressCopy.disabled = false;
    } else {
      energyAddressText.textContent = '付费能量地址未配置';
      energyAddressCopy.disabled = true;
    }
    ...
    energyAddressText.textContent = '配置加载失败';
    energyAddressCopy.disabled = true;
    exchangeAddressText.textContent = '配置加载失败';
...
acceptClaimBtn.addEventListener('click', () => {
  if (!pendingNoticeResolve) {
    return;
  }

  acceptClaimBtn.disabled = true;
  cancelClaimBtn.disabled = true;
  const resolve = pendingNoticeResolve;
  pendingNoticeResolve = null;
  hideClaimNoticeModal();
  resolve(true);
});
...
claimBtn.addEventListener('click', async () => {
  ...
  const accepted = await showClaimNoticeModal();
  if (!accepted) {
    return;
  }
